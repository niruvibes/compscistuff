...
struct chair *make_music(int turns, struct chair *chairs) {
    struct chair *original = chairs;
    struct chair *iterator = chairs;
    int count = 1;
    ...
    iterator->next = original;

    int i = 0;

    iterator = chairs;

    while (i < (count - turns - 1)) {
        iterator = iterator->next;
        i++;
    }

    struct chair *temp = iterator->next;

    chairs = iterator->next->next;
